...
public class MeasurementServiceImpl implements MeasurementService {
    ...
    @Override
    public Optional<Measurement> save(Double weight, Double height, Integer years, Double shouldersSize, Double chestSize, Double hand, Double waist, Double abdomen, Double hip, Double leg) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String username = authentication.getName();
        Optional<User> userOptional = userRepository.findByUsername(username);
        if(userOptional.isPresent()) {
            User user = userOptional.get();
            Measurement measurement = new Measurement(weight, height, years, shouldersSize, chestSize, hand, waist, abdomen, hip, leg, user);
